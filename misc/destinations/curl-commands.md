...
This guide provides sample `curl` commands for testing SAP BTP destination connectivity from a terminal in SAP Business Application Studio. Use these commands to verify that your destination is reachable and returning expected responses before using it with SAP Fiori tools or Service Center.

> **Note**: The `curl` commands in this guide use the `<destination-name>.dest` URL format, which is only accessible from within SAP Business Application Studio. These commands do not work from a local machine or external terminal.
...
- You have an SAP BTP destination configured and active in your subaccount.
- You have `HTML5.DynamicDestination=true` and `WebIDEEnabled=true` set on the destination.

### How `.dest` Routing Works

In SAP Business Application Studio, the URL pattern `https://<destination-name>.dest/` is a proxy provided by the SAP Business Application Studio environment. It resolves to the destination configured in your SAP BTP subaccount and forwards requests to the back-end system using the destination's URL and authentication settings. You do not call the back-end URL directly.

### Escaping `$` in the Shell

When using `curl` from an SAP Business Application Studio terminal, the `$` sign must be escaped with a backslash (`\$`) to prevent the shell from interpreting it as a variable. For example, `$metadata` becomes `\$metadata` in the command.

### Authentication and `username:password`

> **Important**: The `-u username:password` flag in `curl` can only be used when the destination `Authentication` type is set to `NoAuthentication`. SAP Business Application Studio forwards the credentials you provide in the `curl` command directly to the back-end system.
>
> For all other authentication types, such as `BasicAuthentication`, `OAuth2ClientCredentials`, `SAMLAssertion`, the credentials are stored in the SAP BTP destination configuration itself. SAP Business Application Studio handles authentication automatically using those credentials, so passing `-u username:password` in the `curl` command has no effect and must not be used.
...
Use these commands when your destination is configured with `WebIDEUsage=odata_abap`. The destination URL is always the base host. SAP Business Application Studio appends the ABAP catalog paths automatically.
...
> **Note**: These catalog endpoints are only available on ABAP systems. Calling them against a non-ABAP destination (such as a third-party OData service) returns an HTTP 404 Not Found error.

## Commands for `odata_gen` Destinations (Partial)

Use these commands when your destination is configured with `WebIDEUsage=odata_gen` and without a `WebIDEAdditionalData=full_url` property. The destination URL is the base host only, and you append the full service path in the `curl` command.
...
Notice that the service path is not required because it is already included in the destination URL configured in your SAP BTP subaccount.
...
| **HTTP 407 Proxy Authentication Required** | Proxy or Cloud Connector misconfiguration | If `ProxyType=OnPremise`, verify the Cloud Connector configuration. Check the Virtual Host Mapping and Access Control settings. For more information, see [On-Premise Troubleshooting](../onpremise/README.md) guide. |
...
| **HTTP 503 Service Unavailable** | Back-end system is unreachable or unavailable | If `ProxyType=OnPremise`, [investigate the Cloud Connector configuration and connectivity](../onpremise/README.md#validate-connectivity). Verify the Virtual Host Mapping, Access Control settings, and Cloud Connector availability. For more information, see [On-Premise Troubleshooting](../onpremise/README.md) guide. |
...
For persistent issues, run the Environment Check tool in SAP Business Application Studio and attach the generated zip file to your support ticket. For more information, see [Reporting Issues](README.md#reporting-issues).
